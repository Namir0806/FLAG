...
class Net(nn.Module):
    def __init__(self,num_heads):
        ...
        self.layer0 = GATv2Conv(768,int(768/num_heads*2),num_heads=num_heads)
        self.layer1 = GATv2Conv(768*2,int(768*2/num_heads*2),num_heads=num_heads)
        self.layer2 = GATv2Conv(768*2*2,int(768*2*2/num_heads*2),num_heads=num_heads)
        self.layer3 = GATv2Conv(768*2*2*2,int(768*2*2*2/num_heads*2),num_heads=num_heads)
...
def get_graph(graph_filename):
    return dgl.load_graphs(f'../../amrs-to-graphs/truly-all-results-graphs-hk-finbert-plan-{sys.argv[6]}/{graph_filename}.graph',[0])[0][0]
...
def test(model,test_index):
    #memory_file = open('new-GATv2-dim-16x-MSE-results/memory_test_GATv2-dim-16x_'+str(num_of_layers)+'_layers_'+str(num_of_attn_heads)+'_heads_'+sec+'_'+bv+'_lr='+'{:.1e}'.format(learning_rate)+'_plan-'+sys.argv[6]+'_'+sys.argv[3]+'.txt', 'a+')
