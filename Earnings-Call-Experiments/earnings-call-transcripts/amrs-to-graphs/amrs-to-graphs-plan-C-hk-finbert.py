...
try: 
    os.mkdir('truly-all-results-graphs-hk-finbert-plan-C')
except OSError as error: 
    print(error)
track_file = open('tracking_file_all-results_construct_hk-finbert_plan_C_'+str(start_id)+"-"+str(end_id)+".txt",'a+')
...
for i in range(start_id, end_id):
    ...
    while(True):
        ...
        if line[:7] == "# ::tok":
            ...
            #also edge from doc_cls to PREVIOUS snt_cls happens here
    ...
    torch.cuda.empty_cache()
    dgl.save_graphs("truly-all-results-graphs-hk-finbert-plan-C/"+filename[:-3]+"graph",[final_graph])
