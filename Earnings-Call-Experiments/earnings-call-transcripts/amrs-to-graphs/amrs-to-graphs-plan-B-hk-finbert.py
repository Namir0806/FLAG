...
try: 
    os.mkdir('truly-all-results-graphs-hk-finbert-plan-B')
except OSError as error: 
    print(error)
track_file = open('tracking_file_all-results_construct_hk-finbert_plan_B_'+str(start_id)+"-"+str(end_id)+".txt",'a+')
...
for i in range(start_id, end_id):
    ...
    current_snt_root = None
    previous_snt_root = None
    ...
    while(True):
        line = flines[count]
        if line[:7] == "# ::tok":
            #edge from doc_cls to PREVIOUS snt_root happens here
            if current_snt_root is not None:
                n1 = 0
                n2 = current_snt_root.doc_id
            ...
            root_info_list = line[8:].split()
            snt_root_id = root_info_list[0]
            current_snt_root = current_snt_nodes_dict[snt_root_id]
            #connect previous snt_root t current snt_root
            if previous_snt_root is not None:
                n1 = previous_snt_root.doc_id
                n2 = current_snt_root.doc_id
                edge_list1.append(n1)
                edge_list2.append(n2)
        count += 1
    ...
    torch.cuda.empty_cache()
    dgl.save_graphs("truly-all-results-graphs-hk-finbert-plan-B/"+filename[:-3]+"graph",[final_graph])
