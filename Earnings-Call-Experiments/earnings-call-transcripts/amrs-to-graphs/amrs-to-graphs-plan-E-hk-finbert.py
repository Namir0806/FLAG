...
try: 
    os.mkdir('truly-all-results-graphs-hk-finbert-plan-E')
except OSError as error: 
    print(error)
track_file = open('tracking_file_all-results_construct_hk-finbert_plan_E_'+str(start_id)+"-"+str(end_id)+".txt",'a+')
...
for i in range(start_id, end_id):
    ...
    while(True):
        line = flines[count]
        if line[:7] == "# ::tok":
            #edge from doc_cls to PREVIOUS snt_cls and every node in PREVIOUS snt happens here
    ...
    torch.cuda.empty_cache()
    dgl.save_graphs("truly-all-results-graphs-hk-finbert-plan-E/"+filename[:-3]+"graph",[final_graph])
